  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-8 md:py-12">
      ...
      <nav className="text-xs text-gray-500 mb-8 flex items-center gap-2 font-medium">
        <Link prefetch={true} href="/" className="hover:text-brand-main transition-colors">Home</Link>
        <span>/</span>
        <Link prefetch={true} href="/catalog" className="hover:text-brand-main transition-colors">Catalogo Prodotti</Link>
        <span>/</span>
      ...
      <div className="mt-16 w-full">
        <h3 className="text-xl font-bold text-gray-900 mb-6 border-b-2 border-gray-100 pb-3 flex items-center gap-2">
          <svg className="w-6 h-6 text-brand-main" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
          Dettagli Tecnici
        </h3>
        ...
        {techSpecs.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {techSpecs.map((spec: { key: string, value: string }, index: number) => (
              <div key={index} className="bg-white border border-gray-200 rounded-lg p-4 flex flex-col justify-center hover:border-brand-main transition-colors shadow-sm">
      ...
      <div className="mt-20 mb-12">
        <ProductCarousel title="Potrebbe interessarti anche" products={relatedProducts} viewAllLink="/catalog" isLoggedIn={isAuthenticated} />
      </div>
    </div>
  );
}
