    ...
    <div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        <div className="flex justify-between items-center mb-6">
          ...
          <Link href="/account/orders" className="text-[#00C800] hover:underline font-medium">
            Vedi tutti &rarr;
          </Link>
        </div>
            ...
            {recentOrders.map((order) => {
              ...
              return (
                <div key={order.id} className="py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div>
                    <Link href={`/account/orders/${order.id}`} className="inline-block px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded text-sm font-medium transition-colors">
                      Dettagli
                    </Link>
                  </div>
                </div>
