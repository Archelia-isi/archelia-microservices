...
async function check() {
  const users = await prismaDev.equalizzatoreUser.count();
  const prefs = await prismaDev.userPreference.count();
  console.log(`Users in dev: ${users}, Prefs in dev: ${prefs}`);
}

check().catch(console.error).finally(() => poolDev.end());
